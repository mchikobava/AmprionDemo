...
public class GISMappingController : MonoBehaviour
{
        ...
        for (int i = 1; i < lines.Length; i++)
        {
            ...
            if (modelFactory != null)
            {
                ...
                if (icon != null)
                {
                    ...
                    icon.name = "icon";
                    // Combine map-relative scaling with your MW-based scale:
                    // We reuse the same MW logic from the factory by asking it for the multiplier.
                    // If you want to keep the factory pure, recompute the same formula here.
                    float powerScale = 1f;
                    {
                        // replicate your ComputeScaleFromPower(power) from the factory:
                        float s = (power <= 0f) ? 0.8f : Mathf.Clamp(0.6f + Mathf.Sqrt(power) * 0.06f, 0.6f, 2.5f);
                        powerScale = s;
                    }

                    // 1.5% of map XZ diagonal as base icon height; tweak 0.015f to taste
                    FitIconToMap(icon, worldBounds, 0.01f, powerScale);

                    // (Optional) Physics neutralize if needed (prevents "flying" icons)
    ...
    // Make the icon size a fraction of the Germany model.
    // targetFracXZ: e.g. 0.015f means ~1.5% of the map's XZ diagonal.
    // powerScale: additional multiplier from MW (use your ComputeScaleFromPower result).
    void FitIconToMap(GameObject icon, Bounds mapBounds, float targetFracXZ, float powerScale = 1f,
                      float minScale = 0.05f, float maxScale = 50f)
    {
        if (icon == null) return;

        // 1) Measure icon's current size
        ...
        // 2) Reference: map's XZ diagonal
        Vector2 xz = new Vector2(mapBounds.size.x, mapBounds.size.z);
        float mapDiagXZ = xz.magnitude;

        // 3) Choose a target height as a fraction of the map
        float targetHeight = mapDiagXZ * targetFracXZ * powerScale;

        // 4) Uniform scale factor to reach the target height
        float factor = Mathf.Clamp(targetHeight / iconHeight, minScale, maxScale);

        icon.transform.localScale *= factor;

        // (Optional) Recenter local pivot visually to bounds center:
        // var offset = icon.transform.position - iconBounds.center;
        // icon.transform.position += offset; // only if you need it
    }
